...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  3 21:36:00 2020
...
class Alexnet_interpretable_gradcam(nn.Module):
    def __init__(self, num_classes=10):
        super(Alexnet_interpretable_gradcam, self).__init__()
        self.num_classes = num_classes
        self.base_model = nn.Sequential(*list(models.alexnet(pretrained=True).children())[:-2])
        M = 256 #self.base_model[-2].out_channels
        self.sigmoid = nn.Sigmoid()
        self.relu = nn.ReLU(inplace=True)
        
        #self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.fc0 = nn.Linear(256*6*6,1024)
...
class Alexnet_interpretable(nn.Module):
    def __init__(self, num_classes=10):
        super(Alexnet_interpretable, self).__init__()
        self.num_classes = num_classes
        self.base_model = nn.Sequential(*list(models.alexnet(pretrained=True).children())[:-2])
        M = 256 #out_channels
        #self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        ...
        self.classifier = nn.Sequential(    #定义自己的分类层
                nn.Linear(256*6*6, 512),  #512 * 7 * 7不能改变 ，由VGG16网络决定的，第二个参数为神经元个数可以微调
                nn.ReLU(True),
                nn.Dropout(),
                nn.Linear(512, 128),
                nn.ReLU(True),
